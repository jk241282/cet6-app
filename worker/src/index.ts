...
// JWT auth helper for protected routes
...
async function generateToken(userId: number, secret: string): Promise<string> {
  const token = await sign({ userId, exp: Math.floor(Date.now() / 1000) + 7 * 24 * 60 * 60 }, secret);
  return token;
}
...
auth.get('/me', async (c) => {
  ...
  if (!header || !header.startsWith('Bearer ')) return c.json({ error: '未登录' }, 401);
  try {
    const payload = await verify(header.slice(7), 'cet6-dev-secret-change-in-production') as { userId: number };
...
// Extend auth for protected routes
...
vocab.get('/list', async (c) => {
  ...
  if (letter && letter !== 'all') {
    conditions += ' AND v.word LIKE ?';
    params.push(`${letter}%`);
  }
  if (status && status !== 'all') {
    if (status === 'none') conditions += ' AND uv.status IS NULL';
    else { conditions += ' AND uv.status = ?'; params.push(status); }
  }

  const baseFrom = `FROM vocabulary v LEFT JOIN word_meanings wm ON v.id=wm.word_id AND wm.is_primary=1 LEFT JOIN user_vocabulary uv ON v.id=uv.word_id AND uv.user_id=?${conditions}`;

  const countResult = await c.env.DB.prepare(`SELECT COUNT(*) as count ${baseFrom}`).bind(...params).first();
  const total = (countResult as any)?.count || 0;

  const { results } = await c.env.DB.prepare(
    `SELECT v.*, wm.meaning_cn, uv.status as user_status, uv.last_reviewed ${baseFrom} ORDER BY v.word ASC LIMIT ? OFFSET ?`
...
vocab.get('/stats/summary', async (c) => {
  ...
  const totalResult = await c.env.DB.prepare('SELECT COUNT(*) as count FROM vocabulary').first();
  const masteredResult = await c.env.DB.prepare('SELECT COUNT(*) as count FROM user_vocabulary WHERE user_id = ? AND status = ?').bind(userId, 'mastered').first();
  const learningResult = await c.env.DB.prepare('SELECT COUNT(*) as count FROM user_vocabulary WHERE user_id = ? AND status IN (?, ?)').bind(userId, 'learning', 'reviewed').first();

  const total = (totalResult as any)?.count || 0;
  const mastered = (masteredResult as any)?.count || 0;
  const learning = (learningResult as any)?.count || 0;
...
reading.use('*', authVerify);
reading.get('/', async (c) => c.json({ passages: [] }));

// Translation routes (placeholder)
const translation = new Hono<{ Bindings: Bindings }>();
translation.use('*', authVerify);
translation.get('/', async (c) => c.json({ exercises: [] }));

// Writing routes (placeholder)
const writing = new Hono<{ Bindings: Bindings }>();
writing.use('*', authVerify);
writing.get('/', async (c) => c.json({ topics: [] }));
